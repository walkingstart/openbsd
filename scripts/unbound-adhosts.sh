...
_stevenblack="https://cdn.jsdelivr.net/gh/StevenBlack/hosts@master/hosts"
_adreject="https://cdn.jsdelivr.net/gh/Loyalsoldier/v2ray-rules-dat@release/reject-list.txt"
#_anti="https://cdn.jsdelivr.net/gh/privacy-protection-tools/anti-AD@master/anti-ad-domains.txt"
...
# Parse MalwareDom
[[ -n ${_adreject+x} ]] && simpleParse $_adreject $_tmpfile
...
# enohost
[[ -n ${_neohost+x} ]] && ftp -VMo - $_neohost | sed -n '/Start/,$p' | sed -e 's/#.*$//' -e '/^[[:space:]]*$/d' | sed -e 's/^M//' | awk '/^0.0.0.0/ { print $2 }' >> $_tmpfile
# Parse StevenBlack
...
#unbound-checkconf 1>/dev/null &&   unbound-control reload 1>/dev/null
